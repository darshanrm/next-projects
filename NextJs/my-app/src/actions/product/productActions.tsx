...
export  function DeleteProduct(id: string) {
    fetch(`http://localhost:3000/product-api/${id}`, {
        method: 'DELETE',
    })
};
...
export function UpdateProduct(productData: any) {
    fetch(`http://localhost:3000/product-api/${productData.id}`, {
        method: 'PUT',
        mode: 'no-cors',
        credentials: 'include',
        headers: {
...
export default async function AllProduct() {
    const res = await fetch(`http://localhost:3000/product-api`, { cache: 'no-store' })
    return res.json()
}